...
const getLargestSizeID = async (largestItemID) => {
    const res = await axios.get(`https://www.supremenewyork.com/shop/${largestItemID}.json`);
    const data = await res.data;
...
const fetchVariants = async (largestSizeID, region, amount) => {
    try {
        console.log(getCheckoutData(largestSizeID, amount))
...
(async () => {
    ...
    console.log(id)
    const variants = await fetchVariants(id, region, 1000);
